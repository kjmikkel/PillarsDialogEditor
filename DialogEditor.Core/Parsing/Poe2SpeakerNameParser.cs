...
public static class Poe2SpeakerNameParser
{
    ...
    /// Reads Components[0].ChatterPrefix from each GameDataObject and returns a
    /// GUID → ChatterPrefix map (case-insensitive keys). Entries with a missing
    /// or empty Components array, or a null/empty ChatterPrefix, are skipped so
    /// callers never receive blank prefix strings.
    ...
    /// Reads ChatterPrefixes from a file path, handling UTF-8 BOM.
