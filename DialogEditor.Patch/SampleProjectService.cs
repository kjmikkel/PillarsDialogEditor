...
/// The prose the sample project is seeded with — the demo dialogue a first-time user
/// reads, plus the commit subjects they see in the Diff view. Passed in rather than
/// held here because this project references only Core and so cannot reach Loc;
/// DialogEditor.ViewModels.Services.SampleTextFactory builds it from Strings.axaml.
public record SampleTexts(
    string AuthorName,        // git user.name for the seeded repo; shows as commit author
    string EditedLineSuffix,  // appended to the anchor's existing game line
    string AltLineSuffix,     // the experiment branch's competing suffix
    string NewLineText,       // the whole added node's line
    string TranslatorNote,    // the sample translator note on the anchor
    string CommitInitial,
    string CommitReshape,
    string CommitExperiment);
...
public record SampleBuild(
    string ProjectFileName,
    DialogProject Final,
    IReadOnlyList<SampleCommit> Commits,
    string AuthorName);
...
public class SampleProjectService(IGitRunner git)
{
    ...
    // The sample's prose now arrives as SampleTexts: it is the first thing a new user
    // reads, so it is translated like any other UI copy. Only the address below stays
    // literal — .invalid is reserved by RFC 2606 precisely so it can never resolve.
    ...
    public SampleBuild BuildSample(IGameDataProvider provider, SampleTexts texts)
    {
        ...
        var v1 = WithAnchorText(baseSnap, anchor.NodeId, anchor.DefaultText + texts.EditedLineSuffix);
        var p1 = ProjectFrom(name, baseSnap, v1, lang, texts, withNote: false);

        // ── Version 2 (C2, = Final): add a node + link, remove the leaf, add a translator note.
        var newNode = new NodeEditSnapshot(
            newId, false, SpeakerCategory.Npc, "", "", texts.NewLineText, "",
        ...
        var p2 = ProjectFrom(name, baseSnap, v2, lang, texts, withNote: true, noteNodeId: anchor.NodeId);

        // ── Version 3 (C3, experiment): an alternate anchor greeting.
        var v3 = WithAnchorText(v2, anchor.NodeId, anchor.DefaultText + texts.AltLineSuffix);
        var p3 = ProjectFrom(name, baseSnap, v3, lang, texts, withNote: true, noteNodeId: anchor.NodeId);
        ...
        {
            new(texts.CommitInitial,    p1, OnNewExperimentBranch: false),
            new(texts.CommitReshape,    p2, OnNewExperimentBranch: false),
            new(texts.CommitExperiment, p3, OnNewExperimentBranch: true),
        };
        return new SampleBuild(fileName, p2, commits, texts.AuthorName);
    }
    ...
    private static DialogProject ProjectFrom(
        ...
        string lang, SampleTexts texts, bool withNote, int noteNodeId = 0)
    {
        var patch = DiffEngine.Diff(name, baseSnap, current, lang);
        if (withNote)
            patch = patch with
            {
                NodeComments = new Dictionary<int, string> { [noteNodeId] = texts.TranslatorNote }
            };
    ...
    public SampleSeedResult SeedHistory(string repoDir, SampleBuild build)
    {
        ...
        try
        {
            ...
            Run(repoDir, "config", "user.name",  build.AuthorName);
