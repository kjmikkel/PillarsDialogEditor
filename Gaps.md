...
### Spell Checker for Dialog Text ✅ IMPLEMENTED (2026-07-12)

Implemented per the three-layer design below: **WeCantSpell.Hunspell** (tri-licensed,
recorded in `THIRD_PARTY_LICENSES.md`) evaluates user-supplied `.aff`/`.dic` pairs from
the drop-in dictionaries folder (Settings ▸ Spelling: instructions, pinned LibreOffice
source link, open-folder button, detected-languages line); **committed game lexicons**
for 8 languages (`data/lexicons/`, generated by the permanent `tools/lexicon-gen` tool,
English curated — 31 shipped typos/artifacts removed, dialect/British/armor-nautical
vocabulary deliberately kept); and a **global user dictionary** grown via
Add-to-dictionary on sweep rows. Surfaces: spelling joins the detail panel's amber
warning box live (language-routed per text; silently off without a dictionary), and the
sweep window — renamed **"Validate Text…"** — reports Tag and Spelling rows across every
patched conversation and language. Spec:
docs/superpowers/specs/2026-07-11-spell-checker-design.md.

Original design sketch (kept for the record):
...
A word is correctly spelled if it appears in **any** of the three layers.

**Design settled (2026-07-11)** — see
`docs/superpowers/specs/2026-07-11-spell-checker-design.md`: WeCantSpell.Hunspell
engine; manual drop-in `dictionaries/` folder (pinned LibreOffice source link +
instructions + open-folder button in Settings; no network code); surfaces everywhere
via one window (detail-panel amber box + the sweep window generalised to
"Validate Text…" with an issue-type column and an Add-to-dictionary action);
`tools/lexicon-gen/` produces the committed layer-2 lexicons with counts, English
curated first.

**Deferred follow-ups (expected to happen eventually):**
- Inline squiggles in the text boxes (adorner work; shared with token validation).
- Suggestion-apply quick fix (click to accept the proposed word).
- Curated non-English lexicons (tool supports all languages; en curated first).
- Per-project user dictionaries (layer 3 starts global).
- Add-to-dictionary from the detail panel (v1: sweep rows only).
